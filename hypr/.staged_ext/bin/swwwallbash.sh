...
fn_wallbash () {
    ...
    if [[ "${enableWallDcol}" -eq 2 && "${dcol_mode}" == "light" ]] || [[ "${enableWallDcol}" -eq 3 && "${dcol_mode}" == "dark" ]] ; then
        sed -i 's/<wallbash_mode>/'"${dcol_invt}"'/g
                ...
                s/<wallbash_4xa9_rgba(\([^)]*\))>/'"${dcol_1xa1_rgba}"'/g' "${tmp_target}"
    else
        sed -i 's/<wallbash_mode>/'"${dcol_mode}"'/g
                ...
                s/<wallbash_4xa9_rgba(\([^)]*\))>/'"${dcol_4xa9_rgba}"'/g' "${tmp_target}"
    fi
...
if [ "${enableWallDcol}" -eq 0 ] && [[ "${reload_flag}" -eq 1 ]] ; then
    ...
    mapfile -d '' -t deployList < <(find "${hydeThemeDir}" -type f -name "*.theme" ! -name "hypr.theme" -print0)
    deployList+=("$(theme_template_path)")
