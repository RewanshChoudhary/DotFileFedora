...
MPVPAPER_OPTS="load-scripts=no no-audio --loop --hwdec=nvdec --vo=gpu --gpu-api=vulkan --video-sync=display-vdrop --interpolation=no --profile=fast --cache=no --demuxer-readahead-secs=1 --demuxer-max-bytes=16MiB --demuxer-max-back-bytes=4MiB --vd-lavc-threads=1 --framedrop=vo"
...
apply_video_wallpaper() {
  ...
  # Apply video wallpaper using GPU decode/render with lower RAM usage.
  mpvpaper '*' -o "$MPVPAPER_OPTS" "$video_path" &
}
