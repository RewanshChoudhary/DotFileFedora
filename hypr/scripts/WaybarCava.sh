...
# Remove stale cava processes from older script versions.
pkill -f "cava -p ${RUNTIME_DIR}/waybar-cava\\..*\\.conf" >/dev/null 2>&1 || true
...
# Keep pipeline PID so trap can terminate child processes cleanly.
